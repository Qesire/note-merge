...
- **绝不覆盖**：提取前将源文件复制进 vault 作为快照；同名快照追加数字后缀，不覆盖。如果 vault 已有同名笔记，只提供"追加 / 创建版本副本 / 跳过"，无"覆盖/替换"选项。
...
- **衍生笔记标注**：每篇从 ingest 生成的笔记必须在 frontmatter 中同时记录 `source:`（原始文件名）和 `source_snapshot:`（vault 内快照路径），确保可追溯到原始材料。
...
## 原始问题
用户询问 MXFP4 的格式细节、为何采用 E2M1 + E8M0 设计，
以及与 INT4 在实际任务中的精度差异。

## 定义
MXFP4 是一种 4-bit 浮点格式，通常结合块级共享 scale 使用。

## 机制要点
- E2M1 mantissa（2 位指数 + 1 位尾数）
- E8M0 共享 block scale（块内所有元素共享指数）
- 块大小为 32，与硬件对齐

## 未确定点
- 与 NF4 在 DiT 量化场景下的对比实验待完成
- 动态 scale 与静态 scale 的最优选择尚未确定
