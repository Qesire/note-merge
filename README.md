...
把零散的聊天记录、研究报告、随笔手记交给 AI，它会：识别来源类型 → 提取知识 → 匹配 vault 中已有内容分类 → 去重 → 按照 Obsidian PARA+Zettelkasten 规范写入 vault。对于浅层概念笔记，可以读取源码和实验数据，逐层深化为高质量知识笔记。
...
| | chat-export | research-report | casual-note |
|---|---|---|---|
| **例子** | AI 对话记录 | 论文笔记、实验报告 | 随手记、碎片想法 |
| **概念密度** | 高 | 中 | 低 |
| **需要参考？** | 否（对话即参考） | 有条件（需用户补充） | **必须**（否则拒绝深化） |
| **可直接深化？** | ✅ | ⚠️ 需有可追溯引用 | ❌ |
| **默认状态** | draft | draft | draft |
